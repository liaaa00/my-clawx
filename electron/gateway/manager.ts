  ...
  private async startProcess(): Promise<void> {
    ...
    if (app.isPackaged) {
      ...
      if (existsSync(entryScript)) {
        ...
        // Preload gateway-preload.cjs via --require BEFORE the ESM entry point.
        // Passing it as a CLI arg is more reliable than NODE_OPTIONS.
        const preloadPath = path.join(getOpenClawDir(), 'gateway-preload.cjs');
        logger.debug(`Gateway preload check: path=${preloadPath}, exists=${existsSync(preloadPath)}`);
        if (existsSync(preloadPath)) {
          args = ['--require', preloadPath, entryScript, ...gatewayArgs];
        } else {
      ...
      command = 'node';
      // Load gateway-preload.cjs (if it exists) to patch User-Agent for Kimi, etc.
      const preloadPath = path.join(getOpenClawDir(), 'gateway-preload.cjs');
      if (existsSync(preloadPath)) {
        args = ['--require', preloadPath, entryScript, ...gatewayArgs];
      } else {
    ...
    return new Promise((resolve, reject) => {
      const packagedNodeModules = path.join(openclawDir, 'node_modules');
      const nodePathEntries = (process.env.NODE_PATH || '')
        .split(path.delimiter)
        .filter(Boolean);
      const packagedNodePath = app.isPackaged && !nodePathEntries.includes(packagedNodeModules)
        ? [packagedNodeModules, ...nodePathEntries].join(path.delimiter)
      ...
      if (app.isPackaged) {
        ...
        // The gateway-preload.cjs script patches Electron's Module._load wrapper
        // (c._load) which can return null for some modules, breaking Node.js 24's
        // require(esm) support in the ESM translator.
        ...
        if (!existingNodeOpts.includes('--disable-warning=ExperimentalWarning') &&
          ...
          extraOpts.push('--disable-warning=ExperimentalWarning');
        }
        if (extraOpts.length > 0) {
          spawnEnv['NODE_OPTIONS'] = `${existingNodeOpts} ${extraOpts.join(' ')}`.trim();
        }
      }
